...
// Narrow set used for session-propagated name matches (avoid vetoing real names like "Will Smith").
const SESSION_PROPAGATED_STOPWORDS = new Set(["to", "the", "of", "and", "for"]);
...
function applyVeto(spans, fullText, protectedTerms, opts = {}) {
  ...
  for (const span of spans) {
    ...
    // -------------------------------------------------------------------------
    // 0-SESSION) Session-propagated spans (generated by addSessionNameMatches)
    // These are intentionally lower-confidence and should be easy to veto when
    // they touch function words, field labels, or clinical/anatomy tokens.
    // -------------------------------------------------------------------------
    if (!veto && span?.sessionPropagated && NAME_LIKE_LABELS.has(label)) {
      const tokens = norm.split(/\s+/).filter(Boolean);
      if (tokens.some((t) => SESSION_PROPAGATED_STOPWORDS.has(t) || STOPWORDS_CONTEXTUAL.has(t))) {
        veto = true; reason = "session_propagated_stopword_token";
      } else if (activeIndex.anatomySet && tokens.some((t) => activeIndex.anatomySet.has(t))) {
        veto = true; reason = "session_propagated_anatomy_token";
      }
    }
